...
public class Board {
    ...
    private final List<ChessPiece> lightPieces = new ArrayList<>();
    private final List<ChessPiece> darkPieces = new ArrayList<>();

    public Board() {
        locationSquareMap = new HashMap<>();
        Map<Location, ChessPiece> pieces = PieceFactory.getPieces();
        for(int i = 0; i < boardSquares.length; i++) {
            ...
            for(File file : File.values()) {
                ...
                if (pieces.containsKey(newSquare.getLocation())) {
                    ChessPiece piece = pieces.get(newSquare.getLocation());
    ...
    public List<ChessPiece> getLightPieces() {
        return lightPieces;
    }

    public List<ChessPiece> getDarkPieces() {
        return darkPieces;
    }
        ...
        for(int i = 0; i < boardSquares.length; i++) {
            ...
            for(int j = 0; j < boardSquares[i].length; j++) {
                if (boardSquares[i][j].isOccupied()) {
                    ChessPiece piece = boardSquares[i][j].getCurrentPiece();
